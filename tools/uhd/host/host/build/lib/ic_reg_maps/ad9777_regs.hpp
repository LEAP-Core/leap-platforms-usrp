/***********************************************************************
 * This file was generated by /home/kfleming/uhd/host/lib/ic_reg_maps/gen_ad9777_regs.py on Sat Apr  2 14:36:28 2011
 **********************************************************************/
...
public:
    ...
    boost::uint8_t sleep_mode;
    boost::uint8_t power_down_mode;
    enum x_1r_2r_mode_t{
        X_1R_2R_MODE_2R = 0,
        X_1R_2R_MODE_1R = 1
    };
    x_1r_2r_mode_t x_1r_2r_mode;
    boost::uint8_t pll_lock_indicator;
    ...
    boost::uint8_t dataclk_invert;
    boost::uint8_t oneportclk_invert;
    boost::uint8_t iqsel_invert;
    enum iq_first_t{
        IQ_FIRST_I_FIRST = 0,
        IQ_FIRST_Q_FIRST = 1
    };
    ...
    boost::uint8_t idac_fine_gain_adjust;
    boost::uint8_t qdac_fine_gain_adjust;
    boost::uint8_t idac_coarse_gain_adjust;
    boost::uint8_t qdac_coarse_gain_adjust;
    boost::uint8_t idac_offset_adjust_msb;
    boost::uint8_t idac_offset_adjust_lsb;
    ...
    boost::uint8_t qdac_offset_adjust_msb;
    boost::uint8_t qdac_offset_adjust_lsb;
    ...
    boost::uint8_t get_reg(boost::uint8_t addr){
        ...
        switch(addr){
        case 0:
            reg |= (boost::uint8_t(sdio_bidirectional) & 0x1) << 7;
            reg |= (boost::uint8_t(lsb_msb_first) & 0x1) << 6;
            reg |= (boost::uint8_t(soft_reset) & 0x1) << 5;
            reg |= (boost::uint8_t(sleep_mode) & 0x1) << 4;
            reg |= (boost::uint8_t(power_down_mode) & 0x1) << 3;
            reg |= (boost::uint8_t(x_1r_2r_mode) & 0x1) << 2;
            reg |= (boost::uint8_t(pll_lock_indicator) & 0x1) << 1;
            break;
        case 1:
            reg |= (boost::uint8_t(filter_interp_rate) & 0x3) << 6;
            reg |= (boost::uint8_t(modulation_mode) & 0x3) << 4;
            reg |= (boost::uint8_t(zero_stuff_mode) & 0x1) << 3;
            reg |= (boost::uint8_t(mix_mode) & 0x1) << 2;
            reg |= (boost::uint8_t(modulation_form) & 0x1) << 1;
            reg |= (boost::uint8_t(data_clk_pll_lock_sel) & 0x1) << 0;
            break;
        case 2:
            reg |= (boost::uint8_t(signed_input_data) & 0x1) << 7;
            reg |= (boost::uint8_t(two_port_mode) & 0x1) << 6;
            reg |= (boost::uint8_t(dataclk_driver_strength) & 0x1) << 5;
            reg |= (boost::uint8_t(dataclk_invert) & 0x1) << 4;
            reg |= (boost::uint8_t(oneportclk_invert) & 0x1) << 2;
            reg |= (boost::uint8_t(iqsel_invert) & 0x1) << 1;
            reg |= (boost::uint8_t(iq_first) & 0x1) << 0;
            break;
        case 3:
            reg |= (boost::uint8_t(data_rate_clock_output) & 0x1) << 7;
            reg |= (boost::uint8_t(pll_divide_ratio) & 0x3) << 0;
            break;
        case 4:
            reg |= (boost::uint8_t(pll_state) & 0x1) << 7;
            reg |= (boost::uint8_t(auto_cp_control) & 0x1) << 6;
            reg |= (boost::uint8_t(pll_cp_control) & 0x7) << 0;
            break;
        case 5:
            reg |= (boost::uint8_t(idac_fine_gain_adjust) & 0xff) << 0;
            break;
        case 6:
            reg |= (boost::uint8_t(idac_coarse_gain_adjust) & 0xf) << 0;
            break;
        case 7:
            reg |= (boost::uint8_t(idac_offset_adjust_msb) & 0xff) << 0;
            break;
        case 8:
            reg |= (boost::uint8_t(idac_offset_adjust_lsb) & 0x3) << 0;
            reg |= (boost::uint8_t(idac_ioffset_direction) & 0x1) << 7;
            break;
        case 9:
            reg |= (boost::uint8_t(qdac_fine_gain_adjust) & 0xff) << 0;
            break;
        case 10:
            reg |= (boost::uint8_t(qdac_coarse_gain_adjust) & 0xf) << 0;
            break;
        case 11:
            reg |= (boost::uint8_t(qdac_offset_adjust_msb) & 0xff) << 0;
            break;
        case 12:
            reg |= (boost::uint8_t(qdac_offset_adjust_lsb) & 0x3) << 0;
            reg |= (boost::uint8_t(qdac_ioffset_direction) & 0x1) << 7;
            break;
        }
        return reg;
    }
    ...
    boost::uint16_t get_idac_offset_adjust(void){
        return
        (boost::uint16_t(idac_offset_adjust_lsb & 0x3) << 0) |
        (boost::uint16_t(idac_offset_adjust_msb & 0xff) << 2) |
        0;
    }

    void set_idac_offset_adjust(boost::uint16_t reg){
        idac_offset_adjust_lsb = (reg >> 0) & 0x3;
        idac_offset_adjust_msb = (reg >> 2) & 0xff;
    }

    boost::uint16_t get_qdac_offset_adjust(void){
        return
        (boost::uint16_t(qdac_offset_adjust_lsb & 0x3) << 0) |
        (boost::uint16_t(qdac_offset_adjust_msb & 0xff) << 2) |
        0;
    }

    void set_qdac_offset_adjust(boost::uint16_t reg){
